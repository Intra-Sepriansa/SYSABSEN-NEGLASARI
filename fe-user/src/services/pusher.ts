...
export function connectPusher() {
  ...
  EVENT_NAMES.forEach((eventName) => {
    channel?.bind(eventName, async (payload: AttendancePayload) => {
      ...
      const shouldCapture = payload.type === 'in' || payload.type === 'out';
      if (shouldCapture && store.config.photoEnabled) {
        try {
          await captureAndUpload(payload.id, 800, store.config.photoMaxKB);
        } catch (error) {
